        ...
        <div class="card-content p-0">
            <div class="media p-4 mb-0">
                ...
                <div class="media-content">
                    ...
                    <p class="subtitle is-6">@${post.user_name} ${
    post.user_firstname
  } <time datetime="2016-1-1">${post.post_date}</time></p>
            ...
            <div class="content p-4 mb-0 is-small">
                ${post.post_content}
                <p class="has-text-right">${owner && editButton} ${
    owner && deleteButton
  }</p>
            ...
            <footer class="card-footer">
                <a href="#" class="card-footer-item new-comment" onClick="replyPost(${
                  post.post_pk_id
                })">Add comment</a>
...
const formatComment = (comment) => {
  let commentLikes = formatLikes(
    comment.likes,
    comment.comment_pk_id,
    "comment",
    "small"
  );
  return `<details>
...
const vote = async (type, id, element) => {
  ...
  document.querySelector(
    `#like-${id}-${element}`
  ).innerText = `+ ${result.total.likes_likes}`;
  document.querySelector(
    `#disslike-${id}-${element}`
  ).innerText = `- ${result.total.likes_disslikes}`;
};
